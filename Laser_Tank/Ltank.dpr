...
uses
  ...
  CommDlg,
  SysUtils,
  ltank2 in 'LTANK2.PAS',
  lt_sfx in 'LT_SFX.PAS';

{$R ltank.res}
{$D Laser Tank - By Jim Kindley & Delphi Port By Dominique Louis }
{$I- }
...
function DiffBox(Dialog: HWnd; aMsg, WParam: Word;
  LParam: Longint): Bool; stdcall;
var
  temps: array[0..40] of char;
      ...
      begin
        case wparam of
          1:
            begin
              Difficulty := 0;
              if senddlgitemmessage(Dialog, 701, BM_GetCheck, 0, 0) = 1 then
                Difficulty := Difficulty + 1;
              if senddlgitemmessage(Dialog, 702, BM_GetCheck, 0, 0) = 1 then
                Difficulty := Difficulty + 2;
              if senddlgitemmessage(Dialog, 703, BM_GetCheck, 0, 0) = 1 then
                Difficulty := Difficulty + 4;
              if senddlgitemmessage(Dialog, 704, BM_GetCheck, 0, 0) = 1 then
                Difficulty := Difficulty + 8;
              if senddlgitemmessage(Dialog, 705, BM_GetCheck, 0, 0) = 1 then
...
function AboutBox(Dialog: HWnd; Message, WParam: Word; LParam: Longint): Bool;
  stdcall;
var
  H: THandle;
  P: PChar;
  ...
  case Message of
    wm_InitDialog:
      begin
        ...
        H := LoadResource(hInstance, FindResource(hInstance, 'About',
          RT_RCData));
        P := LockResource(H);
...
function WinBox(Dialog: HWnd; Message, WParam: Word; LParam: Longint): Bool;
  stdcall;
var
  H: THandle;
  P: PChar;
...
function HSBox(Dialog: HWnd; Message, WParam: Word; LParam: Longint): Bool;
  stdcall;
var
  ...
  case Message of
      ...
      begin
        ...
        begin
          ...
          if (GHS.moves = 0) or (Game.ScoreMove < GHS.Moves) or
            ((Game.ScoreMove = GHS.Moves) and (Game.ScoreShot < GHS.Shots)) then
          begin
            SetWindowText(GetDlgItem(Dialog, 107),
    ...
    wm_Command: if wparam = 1 then
      begin
        ...
        if stricomp(temps, HS.Name) <> 0 then
        begin
        ...
        case wparam of
          1:
            begin
              ...
              if SendMessage(GetDlgItem(Dialog, 102), bm_GetCheck, 0, 0) <> 0
                then
                SearchRec.mode := 1
              else
                SearchRec.mode := 2;
...
begin
  ...
  case Message of
    ...
    wm_Command:
      begin
        case wparam of
          101: if HiWord(lparam) = lbn_selchange then
            begin
          ...
          104:
            begin { Search }
              ...
              begin
                ...
                begin
                  read(File1, TempRecData);
                  if SearchRec.mode = 1 then
                    strcopy(temps, TempRecData.LName)
                  else
                    strcopy(temps, TempRecData.Author);
                  if strpos(StrUpper(temps), SearchRec.Data) <> nil then
                  begin
                    str(i: 3, temps);
                    strcat(temps, '  ');
                    strcat(temps, Pad(TempRecData.LName, 30));
                    strcat(temps, ' ');
                    strcat(temps, TempRecData.Author);
                    senddlgitemmessage(Dialog, 101, LB_AddString, 0,
                      longint(@temps));
                  end;
                  inc(i);
                end;
  ...
  case Message of
    wm_InitDialog:
      begin
        ...
        begin
          ...
          strcat(temps, ' ');
          strcat(temps, pad(TempRecData.LName, 30));
          if TempHSData.moves > 0 then
          begin
            str(TempHSData.moves: 5, t2);
            strcat(temps, ' ');
            strcat(temps, t2);
            str(TempHSData.shots: 5, t2);
            strcat(temps, '  ');
            strcat(temps, t2);
            strcat(temps, '  ');
            strcat(temps, tempHSData.name);
          end;
          senddlgitemmessage(Dialog, 101, LB_AddString, 0, longint(@temps));
          inc(i);
        end;
    ...
    wm_Command:
      begin
        if wparam = 1 then
          EndDialog(Dialog, 0);
        if (wparam = 101) and (HiWord(lparam) = lbn_selchange) then
        begin
          i := senddlgitemmessage(Dialog, 101, LB_GetCurSel, 0, 0);
          Senddlgitemmessage(Dialog, 101, LB_GetText, i, longint(@temps));
          P := @temps;
  ...
  case Message of
    wm_InitDialog:
      begin
        ...
        begin
          read(File1, TempRecData);
          if File2Ok and (not eof(File2)) then
            read(File2, TempHSData);
          read(File3, TempGHSData);
          str(i: 3, temps);
          strcat(temps, ' ');
          strcat(temps, pad(TempRecData.LName, 30));
          str(TempGHSData.moves: 5, t2);
          strcat(temps, ' ');
          strcat(temps, t2);
          str(TempGHSData.shots: 5, t2);
          strcat(temps, ' ');
          strcat(temps, t2);
          strcat(temps, ' ');
          strcat(temps, pad(tempGHSData.name, 4));
          if File2Ok and (TempHSData.moves > 0) then
          begin
            str(TempHSData.moves: 5, t2);
            strcat(temps, ' ');
            strcat(temps, t2);
            ...
            strcat(temps, t2);
            strcat(temps, ' ');
            strcat(temps, pad(tempHSData.name, 4));
          end;
          senddlgitemmessage(Dialog, 101, LB_AddString, 0, longint(@temps));
          inc(i);
          if File2Ok and (eof(File2)) then
            TempHSData.moves := 0;
        end;
        close(File1);
        close(File3);
        ...
        case wparam of
          1:
            begin
              ...
              if SendMessage(GetDlgItem(Dialog, 101), em_GetModify, 0, 0) <> 0
                then
                Modified := true;
              EndDialog(Dialog, 1);
            end;
  ...
  case Message of
    wm_InitDialog:
      begin
        ...
        PBOpen := true;
        Assign(Book, PBFilename);
        Reset(Book, 1); { Open PLayback file }
        ...
        if strcomp(CurRecData.LName, PBRec.LName) <> 0 then
        begin
          { Do a hard file search for the level name }
          Reset(File1);
          CurLevel := 0;
          while (not eof(File1)) and (strcomp(CurRecData.LName, PBRec.LName) <>
            0) do
          begin
        ...
        begin
          ...
          if messagebox(MainH, temps, 'Laser Tank', mb_OKCANCEL) = 2 then
          begin
...
function MainWindowProc( Window : THandle; aMSG : Cardinal; wParam : Cardinal; lParam: Integer ) : Integer; stdcall;
{function MainWindowProc(Window: HWnd; aMSG, WParam: Word;
...
begin
  ...
  case aMSG of
    wm_create:
      begin
        ...
        begin
          ...
          GetPrivateProfileString('DATA', psRLLN, LevelData, FileName, 100,
            INIFile);
    ...
    wm_Paint:
      begin
        ...
        jk3dframe(pdc, XOffset - 10, YOffset - 10, ContXPos - 5, Box.Bottom - 2,
          true);
        ...
        begin
          ...
          begin
            ...
            TextOut(pdc, ContXPos + 91, 100, CurRecData.LName,
              strlen(CurRecData.LName));
            TextOut(pdc, ContXPos + 91, 150, CurRecData.Author,
              strlen(CurRecData.Author));
            str(CurLevel, temps);
    ...
    wm_KeyDown: if not EditorOn then
      begin
        case wparam of
    ...
    wm_Timer:
      begin
        ...
        if Ani_On then
          inc(AniCount);
        if AniCount = Ani_Delay then
          Animate; { Do Animation }
        if Game.tank.firing then
          MoveLaser; { Move laser if one was fired }
        if PlayBack and (LastKeyPress = 0) and
          (Game.RecP < PBRec.Size) then
        begin
          if speed = 2 then
          begin
            inc(SlowPB);
            if SlowPB = SlowPBSet then
              SlowPB := 1;
          end;
          if (not ConvMoving) and
            ((speed <> 2) or ((speed = 2) and (slowPB = 1))) then
          begin
            inc(Game.RecP);
            ...
            if speed = 3 then
              sendmessage(PlayH, wm_Command, 108, 0);
          end;
        end;
        {if Game.RecP = 11270 then PlayBack := false;}
                            { Check Key Press }
        if (LastKeyPress > 0) and (not (Game.tank.firing or ConvMoving)) then
        begin
          ...
          begin
            inc(Game.RecP);
            if Game.RecP > RecBufSize then
            begin
              if RecBufSize >= RecBufMax then
              begin
        ...
        begin
          ...
          case Game.PF[tank.x, tank.y] of
            ...
            15: if CheckLoc(tank.x, tank.y - 1) then
              begin { Conveyor Up }
                ConvMoving := true;
            ...
            16: if CheckLoc(tank.x + 1, tank.y) then
              begin
                ConvMoving := true;
                UpDateSprite(tank.x, tank.y);
                Inc(tank.x);
            ...
            17: if CheckLoc(tank.x, tank.y + 1) then
              begin
                ConvMoving := true;
                UpDateSprite(tank.x, tank.y);
                Inc(tank.y);
            ...
            18: if CheckLoc(tank.x - 1, tank.y) then
              begin
                ConvMoving := true;
                UpDateSprite(tank.x, tank.y);
                Dec(tank.x);
    ...
    wm_SaveRec:
      begin
        ...
        if temps[0] = ' ' then
          ...
          pbfilename[i] := temps[0];
        if temps[1] = ' ' then
          pbfilename[i + 1] := '0'
        else
          pbfilename[i + 1] := temps[1];
        pbfilename[i + 2] := temps[2];
        if GetSaveFileName(PBfn) then
        begin
          if StrScan(PBfilename, '.') = nil then
            Strcat(PBFileName, '.lpb');
    ...
    wm_MouseMove: if EditorOn then
      begin { if we are editing then check mouse }
        if not drawing then
          exit;
        ...
        begin
          if Game.PF[x, y] <> CurSelBM_L then
            ChangeGO(x, y, CurSelBM_L);
        end
        else
        begin
          if Game.PF[x, y] <> CurSelBM_R then
            ChangeGO(x, y, CurSelBM_R);
        end;
        Exit;
      end;
    wm_LButtonDown: if EditorOn then
      begin
        if LoWord(lparam) > ContXPos then
        begin
          y := (HiWord(LParam) - 250) div (SpBm_Height + 4);
          x := (LoWord(LParam) - (ContXPos + 5)) div (SpBm_Width + 4);
          i := x + (y * EditBMWidth);
          if (i > MaxObjects) or (i < 0) then
            exit;
          CurSelBM_L := i;
          gDC := GetDC(Window);
          PutSelectors;
    ...
    wm_RButtonDown: if EditorOn then
      begin
        if LoWord(lparam) > ContXPos then
        begin
          y := (HiWord(LParam) - 250) div (SpBm_Height + 4);
    ...
    wm_Command:
      case WParam of
        ...
        102:
          begin { Toggle Sound Option }
            Menu := GetMenu(Window);
            if GetMenuState(menu, 102, mf_ByCommand) and mf_Checked <> 0 then
            begin
        ...
        104:
          begin { Toggle Animation Option }
            Menu := GetMenu(Window);
            if GetMenuState(menu, 104, mf_ByCommand) and mf_Checked <> 0 then
            begin
        ...
        105:
          begin { ReStart }
            ...
            begin
              ...
              Game.RecP := 0;
              if Recording then
                BackUp^[UndoP].Tank.Dir := 0; { disable ReStart Undo }
            end;
            Exit;
          end;
        106:
        ...
        109:
          begin { Toggle Remember LL Option }
            Menu := GetMenu(Window);
            if GetMenuState(menu, 109, mf_ByCommand) and mf_Checked <> 0 then
            begin
        ...
        110:
          begin { Undo }
            ...
            if UndoP = 0 then
              UndoP := UndoDepth;
            if BackUp^[UndoP].Tank.Dir = 0 then
              EnableMenuItem(MMenu, 110, mf_ByCommand or mf_Grayed);
                { disable Undo }
            InvalidateRect(Window, nil, false);
          end;
        111:
        ...
        112:
          begin { Restore Position }
            Game := SaveGame;
            InvalidateRect(Window, nil, false);
          end;
        113:
          begin { View High Scores }
        ...
        115:
          begin { Toggle AutoRecord Option }
            Menu := GetMenu(Window);
            if GetMenuState(menu, 115, mf_ByCommand) and mf_Checked <> 0 then
            begin
        ...
        116:
          begin { Toggle SKip Completed Levels Option }
            Menu := GetMenu(Window);
            if GetMenuState(menu, 116, mf_ByCommand) and mf_Checked <> 0 then
            begin
        ...
        123:
          begin { Toggle Record Option }
            if GetMenuState(MMenu, 123, mf_ByCommand) and mf_Checked <> 0 then
            begin
        ...
        225:
          begin { Change Difficulty - Normal Mode}
        ...
        301:
          begin { Hint }
            ...
            begin
              GameOn(false);
              Messagebox(window, CurRecData.Hint, 'Laser Tank Hint', mb_ok or
                mb_iconInformation);
              GameOn(true);
            end;
            Exit;
        ...
        602:
          begin { Load Level [Editor] }
            if (SendMessage(ed1, em_GetModify, 0, 0) <> 0) or
              (SendMessage(ed2, em_GetModify, 0, 0) <> 0) then
              Modified := true;
            ...
            begin
              ...
              begin
                ...
                strcopy(filename, temps);
            end;
          end;
        603:
          begin { Save Level [Editor] }
            if GetSaveFileName(Ofn) then
            begin
              if StrScan(filename, '.') = nil then
                Strcat(FileName, '.lvl');
              Assign(File1, filename);
              Reset(File1);
              if IOResult <> 0 then
              ...
              FreeProcInstance(DProc);
              if (i > 0) and ((i - 1) <= FileSize(File1)) then
              begin
                GetWindowText(Ed1, CurRecData.LName, 30);
                CurRecData.PF := Game.PF;
                CurRecData.PF[game.tank.x, Game.tank.y] := 1;
                  { save Tank in File}
              ...
              begin
                ...
                if ioresult = 0 then
                begin
                  if CurLevel <= FileSize(File2) then
                  begin
                    HS.moves := 0;
  ...
  MainWindowProc := DefWindowProc(Window, aMSG, WParam, LParam);
end;
...
begin
  ...
  begin
    ...
    WindowClass.lpszClassName := 'LaserTankClass';
    WindowClass.lpfnWndProc := @MainWindowProc;
    WindowClass.style := cs_ByteAlignWindow or cs_SaveBits;
  ...
  dwStyle := ws_OverlappedWindow and (not (ws_sizebox or ws_Maximizebox));
  ...
  Ed1 := CreateWindow('EDIT', '', ws_Child or ws_border or es_Left or
    es_AutoHScroll,
    418, 99, 163, 20, Window, 501, hinstance, nil);
  { Author Edit Box - Visable only in Editor Mode }
  Ed2 := CreateWindow('EDIT', '', ws_Child or ws_border or es_Left or
    es_AutoHScroll,
